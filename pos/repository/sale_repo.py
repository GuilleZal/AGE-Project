...
class SaleRepo:
    ...
    def create(self, sale: Sale) -> Sale:
        ...
        cur = self._db.execute(
            """INSERT INTO sales (total, discount, surcharge, payment_method, cash_register_id, created_at)
               VALUES (?, ?, ?, ?, ?, ?)
               RETURNING id, created_at""",
            (sale.total, sale.discount, sale.surcharge, pm, sale.cash_register_id, now),
        )
        row = cur.fetchone()
        sale.id = row["id"]
    ...
    def aggregate_by_period(
        ...
        rows = self._db.execute(
            f"""SELECT strftime('{fmt}', s.created_at) AS period,
